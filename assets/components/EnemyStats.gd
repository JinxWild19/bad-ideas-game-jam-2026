...
var stat_buffs: Array[EnemyStatBuff]
...
signal enemy_health_depleated
signal enemy_health_changed(cur_health: float, max_health: float)
...
func add_buff(buff: EnemyStatBuff):
	stat_buffs.append(buff)
	recalculate_stats.call_deferred()

func remove_buff(buff: EnemyStatBuff):
...
func recalculate_stats():
	...
	for buff in stat_buffs:
		...
		match buff.buff_type:
			EnemyStatBuff.BuffType.ADD:
				if not stat_addends.has(stat_name):
					stat_addends[stat_name] = 0.0
				stat_addends[stat_name] += buff.buff_amount
			
			EnemyStatBuff.BuffType.MULTIPLY:
...
func _on_health_set(new_value: float):
	...
	enemy_health_changed.emit(health, current_max_health)
	if health <= 0.0:
